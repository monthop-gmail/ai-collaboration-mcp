...
```bash
wrangler secret put VIEW_TOKEN        # ไม่ตั้ง = ไม่มีหน้านี้ ไม่ใช่เปิดโล่ง
```
...
| `VIEW_TOKEN` | ไม่บังคับ รหัสของหน้าอ่านที่ `/view` **ไม่ตั้ง = ไม่มีหน้านั้น** แยกจาก `MCP_AUTH_TOKEN` เพราะรหัสนี้อ่านได้อย่างเดียว |
