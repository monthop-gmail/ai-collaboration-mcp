...
export interface Principal {
  ...
  cor?: string;
  /**
   * `kid` ของกุญแจที่ตรวจลายเซ็นใบนี้ผ่าน
   *
   * **สิ่งที่รับประกันคือค่านี้มีอยู่ใน `GATEWAY_JWKS` จริง และกุญแจใบนั้นตรวจลายเซ็น
   * ผ่าน** — ไม่ใช่ว่าเราไม่ได้อ่านจาก header · ตอนตรวจผ่าน `header.kid` กับ `kid`
   * ของกุญแจที่จับคู่ได้เป็นค่าเดียวกันเสมอโดยโครงสร้าง เพราะการค้นใช้ `kid` เป็นกุญแจค้น
   *
   * เขียนแบบนี้เพราะลองทำมิวแทนต์ที่ดึงจาก `header.kid` ตรง ๆ แล้ว **เทสต์ทุกตัวยังเขียว**
   * ซึ่งถูกต้อง — สองทางให้ค่าเดียวกันในทุกเคสที่ไปถึงบรรทัดนี้ได้ · การเขียนว่า
   * "ไม่ได้มาจาก header" จึงเป็นคำอ้างที่ไม่มีอะไรมาหักล้างได้ และคำอ้างที่ล้มไม่ได้
   * ไม่ได้ทำงานอะไร
   *
   * ของที่ทำงานจริงคือ **โทเคนที่อ้าง `kid` ซึ่งไม่มีใน JWKS ไปไม่ถึงบรรทัดนี้เลย**
   * — ตกที่ `unknown_kid` ก่อนตรวจลายเซ็นและก่อนตรวจ `exp` · ข้อนั้นมีเทสต์คุมอยู่
   *
   * `trueforge` ขอไว้ที่ `dis-514ae7a7` seq 74 — audit ของเราบอกได้แค่ว่า
   * *JWT RS256 ผ่านการตรวจ* ส่วนที่ว่าเป็นกุญแจดอกไหนต้องอนุมานข้ามฝั่งจาก
   * `/healthz` ของเขา · **การอนุมานที่แน่นก็ยังไม่ใช่เรคอร์ด** และรอบหมุนกุญแจ
   * ถัดไปจะไม่มีใครย้อนมาเทียบได้ถ้าไม่ได้ลงไว้ตอนนั้น
   *
   * ผ่านเกณฑ์เดียวกับที่ใช้รับ `cor` และปฏิเสธ `jti` — **ลงเมื่อมีคนอ่าน ไม่ใช่ลง
   * เพราะมีค่าให้ลง** · ข้อนี้มีผู้อ่านที่ระบุตัวได้และบอกได้ว่าใช้ทำอะไร
   */
  kid?: string;
}
  ...
  async function verify(token: string, ctx: { operation?: string }): Promise<VerifyResult> {
    ...
    return {
      ...
      principal: {
        ...
        // `kid` ที่ตรวจผ่านจริง ไม่ใช่ค่าที่ผู้เรียกอ้าง
        ...(kid ? { kid } : {}),
      },
    };
  }
