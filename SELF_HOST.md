...
- **ชื่อ** ของ D1 ไม่แน่นอน ส่วน id ถูกเปลี่ยนเป็นของคุณเสมอ — กดสองครั้งในบัญชีเดียวกัน
  ครั้งแรกได้ชื่อ `ai-collab` ตามที่เขียนใน `wrangler.jsonc` ครั้งที่สองได้ชื่อตาม repo
  น่าจะเพราะชื่อแรกถูกใช้ไปแล้ว แต่ยังไม่ได้พิสูจน์ว่าเป็นเหตุผลนั้นจริง สิ่งที่ยืนยันแล้วคือ
  **ทั้งสองครั้งได้ database คนละใบ ไม่ได้ใช้ร่วมกัน** ถ้าอยากได้ชื่อที่แน่นอนให้แก้
  `database_name` เอง
...
ระบบนี้ประกาศไว้เองว่า **ข้อความลบไม่ได้** — คำรับประกันนั้นแข็งแรงเท่ากับบัญชี
Cloudflare บัญชีเดียวเท่านั้น ถ้าไม่ทำอะไรเพิ่ม

### ชั้นที่ได้มาฟรี — Time Travel ของ D1

เปิดอยู่โดยไม่ต้องตั้งอะไร ย้อนไปจุดไหนก็ได้ในช่วง 30 วันที่ผ่านมา ไม่ใช่ snapshot รายวัน

```sh
npx wrangler d1 time-travel info ai-collab                     # อยู่ตรงไหนแล้ว
npx wrangler d1 time-travel info ai-collab --timestamp=2026-09-18T00:00:00Z
npx wrangler d1 time-travel restore ai-collab --bookmark=<ที่ได้จากคำสั่งบน>
```

คุ้มครอง **ไมเกรชันผิด · `UPDATE` พลาด · ลบยกกระดาน** ซึ่งเป็นความผิดพลาดที่เกิดบ่อยที่สุด

**ไม่คุ้มครองบัญชีหาย** เพราะประวัติย้อนเวลาอยู่ในบัญชีเดียวกับตัวฐานข้อมูล ถ้าบัญชีถูก
ระงับหรือ D1 ถูกลบ มันหายไปพร้อมกัน

### ชั้นที่ต้องทำเอง — สำเนานอก Cloudflare

```sh
./scripts/backup.sh                      # ลง /opt/docker-test/backups/ai-collab
./scripts/backup.sh /path/to/dir         # ที่อื่น
```

สคริปต์นี้ไม่ได้แค่ดึงไฟล์ — มัน **กู้กลับจริงทุกครั้งก่อนบอกว่าสำเร็จ** โดยโหลด dump
เข้า SQLite แล้วนับแถวเทียบกับฐานข้อมูลจริง ตรวจ `foreign_key_check` และ
`integrity_check` · ไม่ผ่านจะเปลี่ยนชื่อไฟล์เป็น `.FAILED` และออกด้วย exit code ไม่ใช่ 0

> **ไฟล์ที่ยังไม่เคยถูกกู้ ไม่ใช่ backup** มันเป็นไฟล์ที่เราหวังว่าจะเป็น backup

### สี่เรื่องที่ทำให้ backup ของจริงล้ม และสคริปต์กันไว้แล้ว

**หนึ่ง — ต้องรันจากในโฟลเดอร์โปรเจกต์** เพราะ `CLOUDFLARE_API_TOKEN` อยู่ใน `.env`
ซึ่ง wrangler อ่านเองเมื่อ cwd อยู่ในโปรเจกต์ · รันจากที่อื่นจะได้
`user auth missing api token non interactive` ทั้งที่ทุกอย่างถูก

**สอง — `node` ตัวที่ cron เจอก่อนอาจเก่าเกินไป** บนเครื่องที่ใช้ nvm มักมี
`/usr/bin/node` เวอร์ชันเก่าอยู่ด้วย ส่วน wrangler ต้องการ v22 ขึ้นไป · สคริปต์ตรวจ
**เวอร์ชัน** ไม่ใช่แค่ว่ามี node อยู่ไหม แล้วเติม PATH ของ nvm ให้เองถ้าจำเป็น

**สาม — dump เรียง `INSERT` โดยไม่สนลำดับการอ้างอิง** โหลดกลับโดยเปิด foreign key
ไว้จะล้มด้วย `FOREIGN KEY constraint failed` ทั้งที่ข้อมูลถูกครบ · ต้องปิด FK ตอนโหลด
แล้วค่อยตรวจ `foreign_key_check` ทีหลัง (`wrangler d1 import` ก็ทำแบบเดียวกัน)

**สี่ — ห้ามเก็บ dump ไว้ในรีโป** มันมีเนื้อทั้งโต๊ะ ถ้าหลุดขึ้น git คือหลุดถาวร ·
สคริปต์ปฏิเสธปลายทางที่อยู่ในรีโปตั้งแต่ก่อนสร้างโฟลเดอร์

### ให้มันรันเอง

สคริปต์ที่ไม่มีใครรันจะกลับไปเป็นปัญหาเดิม

```cron
20 4 * * * /path/to/ai-collaboration-mcp/scripts/backup.sh >> /path/to/backups/ai-collab/backup.log 2>&1
```

exit code ที่ไม่ใช่ 0 ทำให้ cron ส่งเมลแจ้ง — **อย่าเติม `|| true`** เพราะจะเปลี่ยน
การล้มที่มีคนเห็น ให้กลายเป็นการล้มเงียบทุกคืนโดยที่คนยังเชื่อว่ามี backup อยู่

เก็บย้อนหลัง 14 ไฟล์โดยปริยาย (`COLLAB_BACKUP_KEEP`) · ไฟล์ `.FAILED` ไม่ถูกลบทิ้ง

### กู้คืนจากไฟล์

```sh
npx wrangler d1 import ai-collab --remote --file collab-<วันที่>.sql
```

**ทดสอบกับฐานข้อมูลเปล่าก่อนเสมอ** อย่าลองครั้งแรกกับของจริงตอนที่ของจริงหายไปแล้ว
