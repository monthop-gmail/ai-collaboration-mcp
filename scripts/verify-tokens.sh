...
# ─────────────────────────────────────────────────────────────────────────
# ไฟล์โทเคนต้องไม่อยู่ในรีโป
#
# `.gitignore` ของเราครอบ `.dev.vars` กับ `.env` **แบบชื่อเป๊ะ** ไฟล์ที่ผู้ใช้ตั้งชื่อเอง
# อย่าง `tokens.txt` หรือสำเนา `.bak` จึงไม่ถูกครอบ แล้วกลายเป็นของที่ commit ได้
#
# `monthop-gmail/botforge` เจอรูปนี้ในรีโปตัวเองเมื่อ 21 ก.ย. — `.gitignore` ครอบ
# `.botforge-deploy.env` แบบชื่อเป๊ะ จึงไม่ครอบไฟล์สำรอง `.bak-<วันที่>` ที่สคริปต์
# ของเขาสร้างเอง ซึ่งถือ API token จริงและรออยู่ใน working tree แบบ untracked
#
# **ด่านที่จุดใช้งานแข็งกว่ารายการที่คนต้องมาต่อให้ครบ** — `backup.sh` ใช้กติกาเดียวกันนี้
# อยู่แล้วกับปลายทางของ dump เพราะ dump มีเนื้อทั้งโต๊ะ
# ─────────────────────────────────────────────────────────────────────────
if [ "$SRC" != "-" ]; then
  ...
  case "$SRC_ABS/" in
    "$REPO"/*)
      echo "ปฏิเสธ: $SRC_ABS อยู่ในรีโป — ไฟล์นี้ถือโทเคนจริง ห้ามเข้า git" >&2
      echo "         ย้ายออกไปนอกรีโปก่อน หรือส่งเข้าทาง stdin: cat ไฟล์ | $0 -" >&2
      exit 2
      ;;
  esac
fi
