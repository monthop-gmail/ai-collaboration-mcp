...
describe("บรรทัดบันทึกของ upstream", () => {
  ...
  /**
   * `kid` ของกุญแจที่ตรวจลายเซ็นผ่าน — ทำให้หลักฐานการหมุนกุญแจครบในฝั่งเดียว
   *
   * `trueforge` ขอไว้ที่ `dis-514ae7a7` seq 74 หลังซ้อมหมุนกุญแจจริง · audit ของเรา
   * ตอนนั้นบอกได้แค่ *JWT RS256 ผ่านการตรวจ* ส่วนที่ว่าเป็นกุญแจดอกไหนต้องอนุมาน
   * ข้ามฝั่งจาก `/healthz` ของเขาว่าตอนนั้นดอกไหน active
   *
   * **การอนุมานที่แน่นก็ยังไม่ใช่เรคอร์ด** — และรอบหมุนถัดไปจะไม่มีใครย้อนมาเทียบได้
   * ถ้าไม่ได้ลงไว้ตอนนั้น เพราะกุญแจเก่าถูกถอนไปแล้วและยิงซ้ำไม่ได้อีก
   *
   * ผ่านเกณฑ์เดียวกับที่ใช้รับ `cor` และปฏิเสธ `jti` — **ลงเมื่อมีคนอ่าน**
   */
  it("kid ลงเมื่อตรวจลายเซ็นผ่าน และไม่มีช่องนั้นเลยเมื่อไม่มี", () => {
    const withKid = auditActor({
      ok: true,
      identity: { name: "s", source: "jwt" },
      principal: { sub: "s", ops: ["read"], cid: "c", kid: "gw-2026-09-24-1" },
    });
    const without = auditActor({
      ok: true,
      identity: { name: "s", source: "jwt" },
    ...
    expect(withKid.kid).toBe("gw-2026-09-24-1");
    expect(without).not.toHaveProperty("kid");
  });
